...
class FileBrowseButton(wx.Panel):
    ...
    def createDialog( self, parent, id, pos, size, style, name ):
        ...
        self.browseButton = self.createBrowseButton(self.iconPath, self.buttonSize)
        boxWrapper.Add( self.browseButton, 0, wx.TOP, -1)
        box.Add( boxWrapper, 0, wx.LEFT|wx.CENTER, 5)
...
#            self.SetSize(size)
    ...
    def createBrowseButton( self, iconPath, buttonSize):
        """Create the browse-button control"""
        button =wx.Button(self, -1, self.buttonText, size=buttonSize)
        if iconPath:
            button.SetBitmap(wx.Bitmap(iconPath), wx.LEFT)
